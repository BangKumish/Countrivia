...
class CountryListAdapter(val dataCountry: List<CountryModel?>?) : RecyclerView.Adapter<CountryListAdapter.MyViewHolder>() {
    class MyViewHolder(view: View): RecyclerView.ViewHolder(view){
    ...
    override fun onBindViewHolder(holder: CountryListAdapter.MyViewHolder, position: Int) {
        holder.tvName.text = dataCountry?.get(position)?.common
        holder.tvCapital.text = dataCountry?.get(position)?.capital
        ...
        holder.itemView.setOnClickListener{
            val name = dataCountry?.get(position)?.common
            Toast.makeText(holder.itemView.context, "$name", Toast.LENGTH_SHORT).show()
        }
    }
